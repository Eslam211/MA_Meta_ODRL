...
class CQRAgent_meta():
    def __init__(self, state_size, action_size, hidden_size=256, device="cpu"):
        ...
        self.BATCH_SIZE = 64
        self.N = 8
        self.quantile_tau = 1 * torch.FloatTensor([(2 * i + 1) / (2.0 * self.N) for i in range(0,self.N)]).to(device)
...
#         self.soft_update(self.network, self.target_net)
    ...
    def learn_cqr_cent(self, experiences, network, target_net, weights):
...
#         self.optimizer.zero_grad()
        ...
        Q_targets = rewards.unsqueeze(-1) + (self.gamma**1 * Q_targets_next.to(self.device) * (1 - dones.unsqueeze(-1)))
        # Get expected Q values from local model
        Q_expected = network(states,weights).gather(2, actions.unsqueeze(-1).expand(self.current_batch_size, self.N, 1))
    ...
    def loss_calc_cent(self,cql1_loss, Q_expected, Q_targets):
        ...
        q1_loss = 1*cql1_loss + 0.5 * loss # alpha = 1
