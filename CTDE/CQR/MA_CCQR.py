...
def cql_loss_cent(q_values, current_action, current_batch_size):
...
def calculate_huber_loss_cent(td_errors, k=1.0):
    """
    ...
    for inner_loop in range(1):
        if inner_loop == 0:
            for batch_idx, experience in enumerate(current_dataloader):
                ...
                for u in range(config.U):
                    loss_CQL, Q_exp, Q_tar = agent_meta[u].learn_cqr_cent((states[:,state_indexing[u]], actions[:,[u]], rewards, next_states[:,state_indexing[u]], dones),net_meta_intermediate[u],net_meta_intermediate_target[u],para_list_from_net[u])
...
def evaluate_MAML(env_test,epoch_number,dataloader_test,Tasks_test_num,if_MAML=False):
    ...
    #             self.optimizer.zero_grad()
                for u in range(config.U):
                    Q_targets_next = target_net_evaluate[u](next_states[:,state_indexing[u]]).detach().cpu()    
                    ...
                    Q_targets = rewards.unsqueeze(-1) + (0.99 * Q_targets_next.to(device) * (1 - dones.unsqueeze(-1)))
                    ...
                    cql1_loss = cql_loss_cent(Q_expected_c, actions[:,[u]], current_batch_size)
...
#         print("Episode: {} | Reward: {} | Q Loss: {}".format(i, eval_reward, loss,))
